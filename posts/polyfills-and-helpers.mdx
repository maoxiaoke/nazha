...
And our goal is clear: **load less code**.
...
If a moudle is resolved to the same path, bundlers like [webpack](https://webpack.js.org/) and [rollup](https://rollupjs.org/guide/en/) will bundle it once.
...
I prefer not to do this. A react application will not know all the NPM packages it depends are fully polyfilled or not. Even though all NPM packages are fully polyfilled, react applications must be fully polyfilled to prevent breakings.
